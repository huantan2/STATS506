...
Since an integer occupies 4 bytes, the memory increses every other integer.  
...
expaneded by a *page* in increments of 2000 bytes as needed.
...
## Profiling with `Rprof` and `Profmem`
...
Here is an example of profling `cor_screen_1` for speed with `Rprof()`.
...
### Using `profviz` to visualize profiling
...
library(profvis)
profvis(invisible(cor_screen_1(yvec, xmat)))
```
